...
void adjusting(char lastTurn, int distance){
	FILE *fp;
	fp = fopen("/dev/servoblaster", "w");
    int d = 0;
    d = distance * 167;
    ...
    dista = getCM();
    char tracingSide;
	if(lastTurn == 'd'){ //just turned right
		//trace left side
		moveSonarLeftRight('a');
		tracingSide = 'a';
		if(dista > 30){
			//trace right side
			moveSonarLeftRight('d');
			tracingSide = 'd';
		}
		//TRACE
		for(int i =0; i <distance; i++){
			if(dista >= 26 && dista <= 29){
				//Move both wheels equally
			}
			else if(dista < 27){
...
void PID(int smallDist, char errorDirect, int angle){
    ...
    if (errorDirect=='a')//error on the left....MOVE RIGHT
    {
        ...
        turnRightAndLeft('d', 90-angle); //turn right b#include <stdio.h>
#include <string>
#include "wiringPi.h"
...
bool getBlueFromCamera(){
    bool final = false;
    std::cout<<"In BLUE Camera"<<std::endl;
...
bool getRedFromCamera(){
    bool final = false;
    std::cout<<"In RED Camera"<<std::endl;
