...
std::vector<Particle> resampleParticles(std::vector<Particle>& oldParticles) {
    bool prefsize;
    char resp;
//    std::cout << "Would you like to resample? (y/n) " << std::endl;
//    std::cin >> resp;
    ...
    if(resp == 'y')
    {
      CDF.resize(oldParticles.size() - 5);  
    ...
    //double u = 1.0/double(oldParticles.size());
        ...
        for(int j=0; j < CDF.size(); j++){
            ...
            p.setWeight(1.0/double(CDF.size()));
...
void drawGuy(cv::Mat img, int x, int y, double angle)
{
    cv::Point pos;
    cv::Point pos2;
    pos.x = x + 25;
...
//     std::cout << "Y value: " << y << std::endl; 
    circle(img, pos, 10, cvScalar(255,192,203), -1, 8);
     if(angle == 0)
    {
        pos2.x = pos.x + 5;
        pos2.y = pos.y;
    ...
    circle(img, pos2, 3, cvScalar(255, 192, 0), -1, 8);

}



void drawParts(std::vector<Particle> &particles, cv::Mat img, cv::Point k, double angle){
...
//     std::cout << "The J value: " << j << std::endl;
//     std::cout << "The weight value: " << weight << std::endl;
